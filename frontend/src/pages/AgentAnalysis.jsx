...
import { Loader2, ShieldCheck, AlertCircle, CheckCircle2, FileText, X, Download, Stethoscope, ChevronLeft, ChevronRight } from 'lucide-react';
import { membersAPI, mlAgentAPI } from '../services/apiService';
...
const normalizeReportDetails = (reportDetails) => {
  ...
  } else if (summaryVal && typeof summaryVal === 'object') {
    summary = summaryVal.text || summaryVal.summary || summaryVal.details || null;
  }
  ...
  // 5. Citations / Clinical Basis Array
    ...
    <html>
      ...
      <body>
        <div class="header">
          ...
          <div class="subtitle">Medicare Risk Stratification & Clinical Verification</div>
        </div>
        ...
        <div class="meta-grid">
          ...
          <div class="meta-item">
            ...
            <span class="meta-value">${member.estimatedRaf || member.predictedRaf || member.risk_score || 'N/A'}</span>
          </div>
          <div class="meta-item">
        ...
        ${citations.length > 0 ? `
          <div class="section">
            <div class="section-title">CITATIONS / CLINICAL BASIS</div>
            ${citations.map(cit => `
              <div class="card">
...
const AgentAnalysis = ({ user, onSignOut }) => {
  ...
  // Fetch Agent reports from PostgreSQL via FastAPI.
  const fetchFlaggedMembers = async () => {
      setLoadingMembers(true);
      setLoadError(null);
      try {
        const result = await mlAgentAPI.getFlaggedMembers(currentPage, 10);
        if (!result.members || result.members.length === 0) {
          setLoadError('No flagged members found in the database.');
          setMembers([]);
          setTotalPages(1);
        } else {
          ...
          const mapped = result.members.map(m => {
            ...
            return {
              ...
              missingHcc: (m.hcc_code != null && m.hcc_code !== '') ? m.hcc_code : '—',
              estimatedRaf: (m.risk_score != null && !isNaN(parseFloat(m.risk_score)))
                ? parseFloat(m.risk_score).toFixed(2)
                : null,
              riskLevel: m.risk_level || null,
              status: m.review_status === 'REVIEW' ? 'Marked for Review' : (m.status || 'Flagged'),
              _raw: m,
          ...
          setReportStatuses(prev => ({
            ...
            ...Object.fromEntries(
              mapped
                .filter(member => member.status === 'Marked for Review')
                .map(member => [member.id, member.status])
            )
          }));

          // Since all flagged members in agent_results have DB results
          const storedReports = {};
          mapped.forEach(m => {
        ...
        setLoadError(`Failed to load flagged members: ${err.message}`);
        setMembers([]);
      } finally {
  ...
  const handleMarkForReview = async () => {
    ...
    try {
      ...
      setReportStatuses(prev => ({ ...prev, [patientId]: 'Marked for Review' }));
      setMembers(prev => prev.map(member => (
        member.id === patientId ? { ...member, status: 'Marked for Review' } : member
      )));
      ...
      <div className="max-w-7xl mx-auto pb-8">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-end mb-6 md:mb-8 gap-4">
          <div>
            ...
            <p className="text-content-muted text-sm md:text-base">Review flagged members requiring clinical verification and detailed medical evidence correlation.</p>
          </div>
        </div>
        ...
        <div className="bg-surface rounded-xl p-6 md:p-8 shadow-sm border border-surface-border card-shadow">
            ...
            <div>
              ...
              <p className="text-sm text-content-muted mt-1">
                {loadingMembers
                  ...
                  : `${members.length} member${members.length !== 1 ? 's' : ''} awaiting Agent Analysis (Page ${currentPage})`}
              </p>
            </div>
          </div>
          ...
          {loadingMembers && (
            <div className="flex items-center justify-center p-12">
              ...
              <span className="ml-3 text-content-muted font-medium">Loading flagged members from database...</span>
            </div>
          )}
              ...
              <table className="w-full text-sm text-left min-w-[900px]">
                <thead>
                  <tr className="text-xs font-semibold text-content-muted uppercase tracking-wider border-b border-surface-border bg-surface-background">
                    ...
                    <th className="py-3 px-4">Mapped HCC</th>
                ...
                <tbody className="divide-y divide-surface-border">
                  {displayedMembers.map((member, idx) => (
                    <tr key={idx} className="hover:bg-surface-background/50 transition-colors">
                      ...
                      <td className="py-4 px-4 font-medium text-content-main">{member.missingHcc}</td>
                      <td className="py-4 px-4">
                        {member.estimatedRaf != null ? (
                           <span className="text-base font-bold text-brand-purple bg-brand-purple/10 px-2.5 py-1 rounded-md">{member.estimatedRaf}</span>
                        ) : (
                      ...
                      <td className="py-4 px-4">
                         {getStatus(member.id) === 'Flagged' && <span className="text-status-danger bg-status-danger/10 font-semibold text-sm px-2.5 py-1 rounded-md">Flagged</span>}
            ...
            <div className="p-6 overflow-y-auto bg-surface flex-1 space-y-6">
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                 <div className="p-4 bg-surface-background rounded-lg border border-surface-border">
                    ...
                    <p className="font-bold text-content-main text-xl">{activeReport.icd || activeReport._raw?.icd10_code || '—'}</p>
                 </div>
                 <div className="p-4 bg-brand-purple/5 rounded-lg border border-brand-purple/20 flex justify-between items-center">
                    <div>
                      <p className="text-xs text-brand-purple font-semibold mb-1 uppercase tracking-wider">Risk Score</p>
                      <p className="font-bold text-brand-purple text-2xl">{activeReport.estimatedRaf || activeReport._raw?.risk_score || 'N/A'}</p>
                    </div>
               ...
               {activeReport._pipelineData ? (
                 ...
                 <div className="text-center py-6 text-content-muted">
                   No detailed report data available for this member yet. Run Agent Analysis to generate a report.
                 </div>
               )}
            </div>
